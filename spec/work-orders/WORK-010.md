...
Status: gate-corrected at `21f2b95` — the Architect's NOT APPROVED verdict at `f5b761d` (two implementation defects) is addressed, the required regression proofs are added, and the complete evidence is rerun green; PR #62 awaiting Architect re-verification.
...
- **Proofs**: `entities-runtime` (14: declaration-validated entity CRUD with every fail-closed class, keyed convergence/divergence/date-normalization/tenant-isolation/tamper detection, the UNKEYED-instance nullable-key contract regression, package registration convergence, the full happy path incl. package assembly + replay convergence, PACKAGE HASH DETERMINISM UNDER A MOVING CLOCK (re-assembly over unchanged authority state converges — the volatile-metadata exclusion regression), the missing-document path + governed chase, expired insurance, noncompliant limits + the NOT_COMPLIANT package gate, vendor correction with the stale-key discrimination, the Zeck reasoning path incl. duplicate-request convergence, the fabricated-AI-claim discrimination, escalation approve/reject, pure-rule determinism), `entities-boundary-checks` (14: the real tree passes; frontier extensions pinned; the public contract + migration invariants; synthetic trees rejected with exact codes — duplicate entity authority, replacement engines, AI selection, forbidden imports, internal imports, importer frontier, schema columns; the prefix discipline; planted AI infra rejected; the check CLI end to end), `entities-concurrency` (5: parallel duplicate document callbacks converge on ONE entity + ONE evidence row; concurrent chasers single-contact with typed recoverable losers; duplicate Zeck requests converge on one reference; the stale-attempt overwrite discrimination with work-state non-regression; parallel same-key entity creation races), `entities.integration` (8, CI-gated: migrations 0001..0012 + schema invariants over real rows, the real-SQL entity flow, UNKEYED instances persist/read/list over real SQL with a NULL row key (the nullable-key mapper regression), the full compliance journey incl. the moving-clock package re-assembly convergence, the governed chase, parallel duplicate callbacks, concurrent chasers, the single-connection pinned-client transaction scope). The faithful `InMemoryEntitiesStore` (race injection) and `buildConstructionApp` (all 12 composed modules + the moving clock handle) are shared through the test helpers.
...
### Architect gate corrections (NOT APPROVED at f5b761d — corrected at 21f2b95)

The Architect's independent verification of PR #62 recorded the authoritative NOT APPROVED / merge-blocked verdict on the PR (gate comment 5519945631): governance/CI evidence was strong, but code review found two substantive correctness defects the delivered proofs did not expose. Both are corrected and the verdict's required regression evidence is added; the complete evidence is rerun at the corrected head.

10. **Nullable `idempotencyKey` broken in the SQL store (gate defect 1)**: the public contract, the store port, the hashing discipline and migration 0012 all permit an UNKEYED instance (`idempotency_key` NULL — the partial unique identity index covers keyed rows only, so unkeyed submissions carry no convergence identity), but the SQL row mapper validated `idempotency_key` as a required string: a valid unkeyed insert was persisted and then failed on read/mapping as `entity-record-tampered`. Correction: the mapper maps NULL to the record's null key exactly as the write path pinned it (a non-string non-NULL key remains tampering). Regression: the live-PostgreSQL proof the verdict required (unkeyed create → raw-row NULL check → read-by-id → list → a second unkeyed submission is a distinct instance → keyed convergence beside unkeyed rows) plus the module-contract twin in-env.
11. **Compliance package hash not deterministic under a moving clock (gate defect 2)**: `assembledAt: now()` rode INSIDE the hashed `packageCore`, so re-assembling unchanged authority state produced a different `packageHash` whenever the clock advanced; the in-memory happy-path replay could not see it (its composed test clock is frozen). Correction: the hash covers the deterministic authority-ledger core ONLY — the observation instant is retained on the returned document OUTSIDE the hashed content. Regression: an advancing-clock in-env proof (assemble twice over unchanged state → identical hash, identical evidence identity, identical deterministic content, `assembledAt` provably moved) and the live journey extension under the live file's advancing clock. Mutation-verified: re-introducing the defect in the compiled output fails EXACTLY the moving-clock proof and nothing else.
12. **Disclosed test-side repair (no product change, the WORK-006/WORK-008 precedent)**: the moving-clock proof exposed that `InMemoryEntitiesStore` read the clock a SECOND time for the row instant instead of pinning the caller's single `input.now` read (the one-read discipline the SQL store honors and the module comment promises) — every in-env test clock was frozen, so the double's divergence from the SQL semantics was invisible. The double now pins `input.now` exactly (the `now` store option is gone with it).
...
- Full suite locally (implementation head `754be5f`) — 898 tests / 784 pass / 0 fail / 114 skipped (107 pre-existing + 7 new live-PostgreSQL proofs, CI-gated).
...
- CI run 33706675186 at evidence head `f5b761d`: both jobs success (898/898) — the head the Architect verification then blocked with the two gate defects.
- Correction head `21f2b95` (gate corrections + regression proofs): `npm run build` PASS; `npm run check` PASS; full suite locally — **901 tests / 786 pass / 0 fail / 115 skipped** (107 pre-existing + 8 WORK-010 live-PostgreSQL proofs, CI-gated).
- CI run 33713020290 at the corrected head `21f2b95`: repository-governance PASS, foundation PASS — **901/901 tests, 0 fail, 0 skipped** (all 115 live-PostgreSQL proofs executed, including the three new regression proofs: the unkeyed real-SQL proof, the moving-clock determinism proof, and the live journey re-assembly assertions).
