...
Run 33660272203 also failed one PRE-EXISTING live proof outside this Work Order's scope: the /zeck same-key parallel convergence proof (`test/zeck.integration.test.ts`, untouched by WORK-008, passed run 33659722044 and the canonical merge run — a nondeterministic flake). Root cause: the proof raced the same logical submission through TWO DISTINCT gateway doubles, which generate divergent foreign execution identities by construction; when the second racer's registration converges between the first racer's registration and reference pin, both dispatch, and the second attach CORRECTLY fails closed REFERENCE_CONFLICT (the store's AC-6 semantics — divergent foreign identities never converge) — a legal outcome the proof's assertion did not tolerate. Test-only harness repair (the WORK-007 createTestPool precedent, disclosed here and in the PR): the same-key convergence section now submits through the SAME gateway double (the double is deterministic per logical intent — executionsByIntent), so both racers dispatch the SAME foreign identity and the convergence proof is timing-independent; the divergent-identity territory remains covered by the proof's own misbehaving-gateway section, and the distinct-prefix gatewayB is retained for the sections that need it. No /zeck production code, module, or store change — the implementation is untouched.
