...
- CI verification: pending push (the implementation environment lost its GitHub credentials across a sandbox reset; the previously provided PAT authenticates as invalid at every GitHub endpoint). PR + completion report follow on credential re-provision.
