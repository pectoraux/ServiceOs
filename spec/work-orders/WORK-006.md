...
The first CI run of the implementation head (run 33675388927, commit 7135b34) failed 8 of the 11 new live-PostgreSQL proofs — every valid inbox insert was rejected by the schema. Root-cause analysis (deriving the auto-generated constraint name `event_inbox_check3` back to its declaration) found one product-side schema defect, fixed in commit f4925e7:

1. **Schema/product contradiction (8 live failures)**: the inbox's rejected-unprocessed invariant was written as a BICONDITIONAL — `(state = 'rejected') = (claim/consumption/failure all null)` — which wrongly requires RECEIVED rows to carry those columns too (received rows also carry them null: `FALSE = TRUE` on every valid delivery insert). The invariant is one-directional (rejected rows are never processed): `state <> 'rejected' OR (all-null)`. The received-all-null shape was already correctly pinned by the separate `received_shape` biconditional, which is why only the received-row inserts failed while the lifecycle transitions, outbox flows and schema-violation proofs passed. The defect was invisible to every in-env test because the in-memory store's read-side shape checks encode the CORRECT semantics — a pure schema-side contradiction (the inverse of WORK-015's PR #28 lesson). All event CHECK constraints are now explicitly named (self-documenting violations, name-pinned proofs), and the live schema test now proves the missing direction: every LEGAL lifecycle shape is admissible as a raw row (received, rejected, intended) — the regression that would have caught this before the module-level proofs.

2. **Test-side corrections (CI run 33677458499, commit f722c13; no product change)**: (a) the schema test's received+claim insert violates both `received_shape` and `claim_present` — PostgreSQL may report either, the assertion now accepts both names; (b) the moving-clock inbox side reused the outbox part's terminally-observed interaction — a delivery result for it correctly fails `OBSERVATION_STATE_INVALID` (terminal observation semantics), the test now uses a fresh dispatched interaction; (c) the cross-tenant proof wrapped `processInboxEvents` for the other tenant's EMPTY claimable set in a fail-closed helper, but the tenant-isolated behavior is a legitimate success with zero outcomes — asserted directly, plus the foreign event stays `received`.
