...
def main():
    ...
    if list(dep["futureGeneration"])!=road["sequence"]: fail("futureGeneration key order != roadmap sequence")
    if not set(dep["futureGeneration"])<=all_ids: fail("futureGeneration contains unknown Work Order identity")
    waves=sum(road["parallelWaves"],[])
    if set(waves)!=set(road["sequence"]) or len(waves)!=len(set(waves)): fail("parallel waves do not cover each future Work Order exactly once")
    current_complete=set(dep["currentGeneration"].get("complete",[])); current_inflight=set(dep["currentGeneration"].get("inFlight",[])); future=set(dep["futureGeneration"])
    if current_complete & current_inflight: fail("currentGeneration overlaps complete and inFlight")
    if (current_complete|current_inflight)&future: fail("a Work Order appears in current and future generation")
    if current_complete|current_inflight|future!=all_ids: fail("generation state does not cover exactly all Work Order files")
    graph=dep["futureGeneration"]
    for wid,deps in graph.items():
        for d in deps:
            if d not in all_ids: fail(f"{wid} depends on unknown {d}")
    ...
    for wid,r in records.items():
        ...
        if "branch" not in r or "dependencies" not in r: fail(f"program-state {wid} missing branch/dependencies")
        if r["dependencies"]!=graph.get(wid,[]): fail(f"program-state {wid} dependencies != canonical dependency-state")
    ...
    if not planned_next: fail("frontier plannedNext must not be empty")
    for wid in planned_next:
        if wid not in future: fail(f"frontier plannedNext contains non-future Work Order {wid}")
    ...
    for p in (ROOT/"spec/work-orders").glob("WORK-*.md"):
        ...
        if parse_requires(text)!=graph.get(p.stem,[]): fail(f"{p.name} Requires list != canonical dependency-state")
    for wid,r in records.items():
    ...
    expected=["SENSE","UNDERSTAND","PLAN","CHECK","EXECUTE","VERIFY","REVIEW","MERGE","FINALIZE","LEARN","SENSE"]
    if gov.get("implementationControlLoop")!=expected: fail("unexpected implementation control loop")
